...
Personas are small, forkable AI agents you write yourself: a markdown file at `{library}/personas/{slug}.md` with a bit of YAML frontmatter and a prompt-template body. Tiro ships three defaults — `devils-advocate` (counterpoint notes on an article), `daily-themes` (a digest section), `research-brief` (a wiki-page draft from a search query) — copied into your library on first use so you can edit them freely.

**Frontmatter contract:** `name`, `scope` (`article` / `day` / `query` / `library`), `output` (`note` / `digest_section` / `wiki_page` / `tier_suggestion`), plus optional `version`/`schedule`/`tier`. The body is a template over a small, fixed placeholder set (`{{article}}`, `{{highlights}}`, `{{query}}`, `{{day_articles}}`, `{{wiki_page}}`) — which placeholders are available depends on the persona's scope. Anything else (unknown key, unknown placeholder, a placeholder outside its scope) fails to load, loudly, in the `/agents` persona list — there's no silent partial run.
...
> **⚠️ Renaming a file is NOT tracked as a rename.** The reconcile engine has no rename detection: it sees a renamed file as one file disappearing and a different one appearing, and processes that as a delete followed by a fresh ingest, in the same pass. The old article's highlights, notes, ratings, and read-state are permanently dropped (via the same cleanup path as deleting from the UI), any cached audio for it is deleted, and the renamed file comes back as a brand-new `external` article with none of that history. The article *content* survives (it's still the same text, just a new article record) — it's the Tiro-side annotation/rating state that's lost. **Don't rename article files from outside Tiro** if you care about keeping that state. Rename-aware reconciliation (matching by content rather than by filename) is planned for a later sync milestone, not built yet.

- **Edit an article's body** — the change is picked up, the article re-indexes and re-embeds, and any highlights in that article are re-checked against the new text (shifted anchors follow the edit; anchors that can no longer be located are flagged, never silently dropped or guessed at). Note that this does **not** regenerate the AI-derived summary or tags — those were computed once at ingest time and stay as they were, so they can go stale relative to a heavily-edited body until the article is re-ingested (or `POST /api/articles/{id}/analysis` is re-run, for the ingenuity/trust analysis specifically).
