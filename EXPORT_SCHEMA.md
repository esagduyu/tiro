...
| `sources` | `id, name, domain, email_sender, source_type, is_vip, created_at` | All sources, unfiltered (referenced sources for filtered articles are a subset of this). As of Sync S1, rows also carry `uid` (a TEXT ULID, backfilled + stamped at all creation sites) — additive, since `_get_sources` is a plain `SELECT *`. |
